...
export function SpreadsheetStatusBar({ summary }: SpreadsheetStatusBarProps) {
  ...
  return (
    <div
      ...
      className="flex min-h-9 shrink-0 items-center gap-2 overflow-x-auto border-t border-surface-200 bg-surface-50 px-3 text-xs text-surface-600 dark:border-surface-800 dark:bg-surface-900 dark:text-surface-300"
    >
      {items.map(([label, value], index) => (
        <span
          ...
          className={
            index === 0
              ? "inline-flex h-7 shrink-0 items-center rounded-md border border-surface-200 bg-white px-2 dark:border-surface-700 dark:bg-surface-950"
              : "shrink-0 px-1"
          }
        >
