...
export function WriterFlowChart({ dayIndices, words, height = 164, tone = "document", label }: WriterFlowChartProps) {
  ...
  const option = useMemo<EChartsCoreOption>(() => {
    ...
    const accent = tone === "journal" ? (isDark ? "#67d2c8" : "#0f9f94") : (isDark ? "#d8b45f" : "#b8872e");
    const accentBright = tone === "journal" ? (isDark ? "#8de7dd" : "#36c6ba") : (isDark ? "#f0cf75" : "#d5a142");
    const accentSoft = tone === "journal" ? (isDark ? "#1e6b67" : "#b7f1eb") : (isDark ? "#7a5d25" : "#f1d894");
    const textMuted = isDark ? "#8ea0b8" : "#64748b";
    const gridLine = isDark ? "rgba(148, 163, 184, 0.14)" : "rgba(148, 163, 184, 0.22)";
    const chartLabel = label || t("chart.wordsWritten");
      ...
      series: [
        {
          ...
          emphasis: {
            itemStyle: {
              shadowBlur: 14,
              shadowColor: tone === "journal" ? "rgba(15, 159, 148, 0.28)" : "rgba(216, 180, 95, 0.32)",
            },
          },
        },
        {
            ...
            color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
              {
                ...
                color:
                  tone === "journal"
                    ? isDark
                      ? "rgba(103, 210, 200, 0.32)"
                      : "rgba(15, 159, 148, 0.2)"
                    : isDark
                      ? "rgba(216, 180, 95, 0.38)"
                      : "rgba(184, 135, 46, 0.24)",
              },
              { offset: 1, color: tone === "journal" ? "rgba(15, 159, 148, 0)" : "rgba(216, 180, 95, 0)" },
            ]),
          },
        },
      ],
    };
  }, [dayIndices, isDark, label, lang, t, tone, words]);

  return <ReactEChartsCore echarts={echarts} option={option} style={{ width: "100%", height }} notMerge lazyUpdate />;
}
