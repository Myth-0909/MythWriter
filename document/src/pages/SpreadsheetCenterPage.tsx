...
export function SpreadsheetCenterPage({ onOpenSpreadsheet }: SpreadsheetCenterPageProps) {
  ...
  return (
    <main className="min-h-0 flex-1 overflow-y-auto bg-surface-50 px-5 py-6 sm:px-6 lg:px-8 dark:bg-surface-950">
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6">
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-4">
          {filtered.map((spreadsheet) => (
            <article
              key={spreadsheet.id}
              className="flex min-h-[168px] flex-col justify-between rounded-xl border border-surface-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-md dark:border-surface-800 dark:bg-surface-900 dark:hover:border-brand-500/40"
            >
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1.5 line-clamp-2 min-h-[40px] text-sm leading-5 text-surface-500 dark:text-surface-400">
        ...
        {!loading && !loadError && filtered.length === 0 && (
          <div className="flex min-h-[280px] flex-col items-center justify-center rounded-xl border border-dashed border-surface-300 bg-white p-8 text-center dark:border-surface-700 dark:bg-surface-900">
            ...
            <p className="mt-2 max-w-md text-sm leading-6 text-surface-500 dark:text-surface-400">{t("sheets.emptyDesc")}</p>
          </div>
        )}

        {loading && (
          <div className={cn("grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-4")}>
            {[0, 1, 2, 3].map((item) => (
              <div key={item} className="h-[168px] animate-pulse rounded-xl bg-surface-100 dark:bg-surface-900" />
            ))}
          </div>
        )}
      </div>
